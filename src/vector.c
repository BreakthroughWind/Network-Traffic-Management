...
void ds_vector_free_no_data(struct DSVector *vec)
{
    free(vec->data);
    free(vec);
}
...
void ds_vector_append(struct DSVector *vec, void *data)
{
    ds_vector_maybe_expand(vec);
    vec->data[vec->size++] = data;
}

void ds_vector_insert(struct DSVector *vec, void *data, int32_t index)
{
    int32_t i;
...
void ds_vector_remove(struct DSVector *vec, int32_t index)
{
    int32_t i;
...
void ds_vector_set(struct DSVector *vec, void *data, int32_t index)
{
    if (index >= vec->size)
        return;
...
void ds_vector_swap(struct DSVector *vec, int32_t i, int32_t j)
{
    void *temp;
...
void ds_vector_map(struct DSVector *vec, void(func)(void *))
{
    int32_t i;
 ...
 */
void ds_vector_sort(struct DSVector *vec, int32_t(compare)(void *, void *))
{
